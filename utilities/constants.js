...
const FORMAT_RESPONSE_FIELD = (message) => message.replace('The ', '').replace(' field is required.', '')
...
module.exports = {
  ...
  GENERAL_FUNCTIONS: {
    ...
    FORMAT_ERROR: (error) => {
      ...
      if (message.indexOf('foreign key') > -1) message = 'cannot delete due to foreign key'
    ...
    FORMAT_SHAGO_ERROR: (response) => {
      switch (response.status) {
        case '301':
          return `${FORMAT_RESPONSE_FIELDS(response.required_fields).join(', ')} missing from the request`
        case '310':
